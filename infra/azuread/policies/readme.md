# AzureAD Custom Policies for Alcumus

 This folder hosts all the custom policies applied for Alcumus' AzureAD B2C integration for authentication with auth-service.

 ## Working with Custom Policies

 Following are prerequisites to work with AAD B2C Custom Policies

 - **powershell**: required to upload custom policies to Alcumus Azure account.
 - **ngrok**: required to intercept AAD B2C actions and relay the calls to locally running service.
 - Make sure you have an `ngrok` account setup.

 To work with AAD B2C Custom Policies locally:

 - Update your `.env` file for these environment variables:
   - `INFRA_ENVIRONMENT_NAME`: Your initials as value is fine. This gets injected into custom policy file names when they get uploaded to environments. Final file name would look like `B2C_1A_{Your Initials}_{PolicyFileName}.xml`.
   - `AZURE_AD_COMBINED_SIGNUP_SIGNIN_POLICY_NAME`: Set this value to `B2C_1A_{Your Initials}_SIGNUP_SIGNIN.xml`

 - Upload your policy changes to Azure account using `./infra/azuread/scripts/uploadDeveloperPolicies.sh`. You can go to your `Azure account > Azure AD B2C > Identity Experience Framework` and see your policies uploaded. (You may need to switch to QA-BI directory if you're on some other directory)

 - Run services locally

 - Open a new terminal and run `ngrok http 8000` to setup ngrok to intercept service calls from AAD B2C

 - At this point you have setup AAD B2C Custom Policies working against your local services and you can verify your changes.


 ## Azure Documentation on Custom Policies
